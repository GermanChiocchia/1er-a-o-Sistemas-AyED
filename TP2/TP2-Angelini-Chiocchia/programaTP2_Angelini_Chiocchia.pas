{Trabajo practico N°2 Algoritmos y estructura de datos 2020-Chiocchia German, Angelini Luciano}
...
var
    ...
    enf:enfe;
    matriz_enf_sint:array[1..we,1..es]of integer;
...
procedure validar_cod(var cod:char);
var
   l:integer;
   i:char;
          begin
          ...
          begin
               ...
               begin
                    ...
                    begin
                         if le = i then
                         begin
                              if (le='C')or (le='E')or (le='F')or (le='J')or (le='L')or (le='M')or (le='N')or (le='P')or (le='R')or (le='S') then
                              l:=1;
                         end;
                    end;
...
begin
     ...
     while r<>s do
     begin
           while (length(dni)>8) and (length(dni)<>0) do                 {longitud dni}
           begin
                writeln('Ingrese un dni valido sin puntos intermedios: ');
                readln(dni);
           end;
           ...
           begin
               for j:='0' to '9' do                                 {caracteres dni}
                    if dni[i]=j then
                                r:=r+1;
           end;
